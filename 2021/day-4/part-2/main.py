...
class BingoGame():
    ...
    def play(self):
        ...
        results = list(map(lambda c: c.check_draw(draw), self.cards))
        ...
        # Remove completed cards in reverse order to preserve indexing
        results.reverse()
        for x, res in enumerate(results):
            # Invert to match cards index
            i = len(results) - 1 - x
            if res:
                # print(f'Card {i} of {len(self.cards) -1} has won!')
                self.winning_card = self.cards.pop(i)
